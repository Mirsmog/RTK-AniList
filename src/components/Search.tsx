...
const Search: FC<SearchProps> = ({ searchRef, visible }) => {
  ...
  const prevSearchValue = useAppSelector(
    state => state.filter.past.length > 0
      ? state.filter.past[state.filter.past.length - 1].searchValue
      : '')
  ...
  return (
    ...
    >
      ...
      <div className={`rounded-xl absolute bg-slate-700 w-full top-[82%] z-20 p-6 pr-0 shadow-slate-900 drop-shadow-md shadow-xl rounded-t-none border-slate-600 border-2 overflow-hidden transition-opacity duration-300 ${(!visible || resultList.length == 0) && 'opacity-0 h-0 py-0'} ${hasNextPage ? 'pb-2' : ''} searchTarget`}>
        ...
        {
          hasNextPage && <Link onClick={checkClearDate} to={`search/${searchValue}`} className='showAll inline-block pt-2 text-md text-info hover:text-indigo-500 hover:underline'>show All</Link>
        }
      </div>
    </div >
  );
