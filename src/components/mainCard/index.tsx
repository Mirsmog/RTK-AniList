...
const MainCard: FC<ISearchProps> = ({ image, title, id, isSearchCard, releaseDate, subOrDub }) => {
  const [added, setAdded] = React.useState<boolean>(false);
  const { data: bookmarks = [] } = useGetBookmarksQuery();
  const [addToBookmark, { isLoading: addIsLoading }] = useAddBookmarkMutation();
  const [deleteBookmark] = useDeleteBookmarkMutation();

  function findIdByNameId(arr: Bookmarks[], nameId: string) {
    for (let i = 0; i < arr.length; i++) {
      if (arr[i].nameId === nameId) {
        return arr[i].id;
      }
    }
    return null;
  ...
  React.useEffect(() => {
    if (findIdByNameId(bookmarks, id)) {
      setAdded(true);
    }
  }, [bookmarks]);
  ...
  const setBookmark = async (id: string) => {
    if (findIdByNameId(bookmarks, id)) {
      setAdded(false);
      await deleteBookmark(findIdByNameId(bookmarks, id));
    } else {
      !addIsLoading && setAdded(true);
      !addIsLoading && (await addToBookmark({ nameId: id }));
    }
  };
    ...
    <li className='w-full flex gap-4 relative' key={id}>
      ...
      <div className='w-full'>
        ...
        <div className='grow relative'>
          ...
          <span className={`${subOrDub === 'dub' ? 'text-info' : 'text-indigo-500'} font-bold`}>
            {subOrDub}
          </span>
          <svg
