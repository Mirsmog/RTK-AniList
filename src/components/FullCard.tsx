...
const FullCard: React.FC<FullCardProps> = ({
  ...
  return (
    <div className='card card-side bg-base-100 shadow-xl'>
      <figure className='w-full max-w-[230px] justify-start'>
        <img className='object-cover self-start w-full h-auto' src={image} alt='Movie' />
      </figure>
      <div className='card-body p-6 pr-8'>
        <div className='flex justify-between'>
          ...
          <div
            className={`capitalize ${
              subOrDub === 'dub' && 'bg-indigo-500'
            } bg-accent p-[1px] px-3 rounded-full self-start text-white ml-2 text-basic align-top font-semibold`}
          >
            {subOrDub}
          </div>
        </div>
        ...
        <div
          ...
          dangerouslySetInnerHTML={{ __html: shortText(description, 300) }}
        ></div>
